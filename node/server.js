...
app.post('/compile/:name', function (req, res) {
    let child = (0, child_process_1.exec)('bash build-release.sh --module="logwire"', { cwd: getFolderPath(req.params.name) });
...
app.post('/execute/:name', function (req, res) {
    ...
    (0, child_process_1.exec)(`pm2 start ./${req.params.name}/execute.backend.js --name ${req.params.name}_backend --no-autorestart`);
    (0, child_process_1.exec)(`pm2 start ./${req.params.name}/execute.gateway.js --name ${req.params.name}_gateway --no-autorestart`);
